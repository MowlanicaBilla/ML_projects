...
class LlamaCPPInvocationLayer(PromptModelInvocationLayer):
	...
	def _ensure_token_limit(self, prompt: Union[str, List[Dict[str, str]]]) -> Union[str, List[Dict[str, str]]]:
		"""
		Ensure that the length of the prompt and answer is within the maximum token length of the Prompt
		:param prompt: Prompt text to be sent to the generative model
		"""
		if not isinstance(prompt, str):
			raise ValueError(f"Prompt must of type: String but got {type(prompt)}")
		...
		if len(tokenized_prompt) + self.max_length > context_length:
			logger.warning(
				"The prompt has been truncated from %s tokens to %s tokens so that prompt length doesn't exceed our limitations"
	...
	def invoke(self, *args, **kwargs):
		"""
		It takes a prompt and returns a list of generated text using underlying model.
	...
	def supports(cls, model_name_or_path:str, **kwargs) -> bool:
		"""
		Checks if the below model is supported by the invocation layer.

		:param model_name_or_path: The name or path of the model
		:param kwargs: Additional keyword arguments passed to the underlying model which
		if the model is supported
		:return: True if the invocation layer supports the model, False otherwise
		"""
